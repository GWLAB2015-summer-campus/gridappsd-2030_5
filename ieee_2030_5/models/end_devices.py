...
from . import (EndDevice, Registration, RegistrationLink, DeviceInformation, DeviceInformationLink,
               DeviceStatusLink, PowerStatusLink, SubscriptionListLink, ConfigurationLink, FileStatusLink,
               EndDeviceList, DeviceCategoryType)
...
class EndDevices:
    ...
    def register(self, device_category: DeviceCategoryType, l_fid: Lfid, pin_code=999):
        ...
        power_status_link = PowerStatusLink(href=hrefs.edev_power_status_fmt.format(index=new_dev_number))
        file_status_link = FileStatusLink(href=hrefs.edev_file_status_fmt.format(index=new_dev_number))
        dev_info_link = DeviceInformationLink(href=hrefs.edev_info_fmt.format(index=new_dev_number))
        sub_list_link = SubscriptionListLink(href=hrefs.edev_sub_list_fmt.format(index=new_dev_number))
        ...
        registration = Registration(date_time_registered=ts,
                                    p_in=999)

        dev_indexer = EndDeviceIndexer(index=new_dev_number,
                                       end_device=dev)
                                       #,
                                       #registration=registration)
